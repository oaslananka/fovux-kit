...
Fovux v1.0.0 requires a bearer token for every HTTP route except `/health`. The token is persisted under `FOVUX_HOME/auth.token` and read directly by the VS Code extension.
