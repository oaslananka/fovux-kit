...
from fovux.http.tool_proxy import available_tools, policy_for_tool, HttpToolPolicyError
...
def test_support_bundle_generation(mock_fovux_home: Path) -> None:
    ...
    log_file.write_text("INFO tool_start fovux_doctor\nINFO tool_end fovux_doctor\n", encoding="utf-8")
    
    # Add a secret to env to test redaction
    os.environ["PYPI_TOKEN"] = "sensitive_secret_token"
    ...
    with zipfile.ZipFile(zip_path, "r") as z:
        ...
        assert manifest["env_summary"]["env_vars"]["PYPI_TOKEN"] == "[REDACTED]"
