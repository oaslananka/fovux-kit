"""Validate governance lifecycle, ADR, and contributor ladder docs."""
...
def main() -> int:
    ...
    for phrase in ["Triage", "Ready", "In progress", "Waiting review", "Done/closed", "ADR"]:
        if phrase not in lifecycle:
            failures.append(f"Issue lifecycle missing {phrase}")
    ladder = _read(ROOT / "docs" / "contributor-ladder.md")
    for phrase in ["Contributor", "Regular contributor", "Maintainer", "Agent contributor", "validation"]:
    ...
    for path in [".github/labels.yml", ".github/workflows/sync-labels.yml", ".github/workflows/auto-label.yml", ".github/workflows/stale.yml", ".github/CODEOWNERS"]:
        if not (ROOT / path).exists():
            failures.append(f"Missing governance automation file: {path}")
    if failures:
