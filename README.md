...
Fovux is a local-first vision workbench for YOLO datasets, training, evaluation, export, and edge inference. It combines the Python backend package `fovux-mcp` 1.6.0, the npm wrapper `fovux-mcp` 1.6.0, and the VS Code companion `Fovux Studio` 1.5.0 to streamline the computer vision engineering lifecycle.
...
Fovux MCP 1.6.0 exposes 47 local tools across dataset inspection, validation, active learning, training, evaluation, export, quantization, inference, benchmarking, run management, policy/audit, and support-bundle workflows.
